/*
  ...
  ✅ Basic ops: +, -, *, /, sqrt
  ✅ NaN / Inf / zero semantics broadly consistent with Intel/IEEE expectations:
     - NaNs propagate (quieted)
     - inf +/- inf of opposite sign -> NaN
     - 0 * inf -> NaN
     - x / 0 -> inf (signed)
     - 0 / 0 -> NaN
     - sqrt(negative) -> NaN
     - preserves sign for infinities; zeros mostly follow common IEEE practice
  ...
  Build test:
    g++/clang++ -std=c++17 -DX87_EXT80_TEST x87_ext80.hpp && ./a.out
    cl /std:c++17 /EHsc /DX87_EXT80_TEST x87_ext80.hpp
*/
...
// atan2(y, x) — portable, no host-FPU dependency
// Range: (-pi, pi]
static X87_EXT80_FORCEINLINE ext80 atan2_big(const ext80& y_in, const ext80& x_in) {
  ext80 y = y_in; ext80 x = x_in;
  y.canonicalize();
  x.canonicalize();

  // NaN propagation -> unordered semantics
  if (y.is_nan()) return y;
  if (x.is_nan()) return x;

  // Handle infinities using quadrant rules
  if (y.is_inf() || x.is_inf()) {
    // atan2(±inf, ±inf) -> ±3pi/4, ±pi/4
    // atan2(±inf, finite) -> ±pi/2
    // atan2(finite, ±inf) -> ±0 or ±pi depending on signs
    const ext80 pi      = ext80::from_double(3.141592653589793238462643383279502884);
    const ext80 half_pi = ext80::from_double(1.570796326794896619231321691639751442);
    const ext80 quarter_pi = ext80::from_double(0.785398163397448309615660845819875721);

    if (y.is_inf() && x.is_inf()) {
      // atan2(signY*inf, signX*inf)
      if (!x.sign) return y.sign ? -quarter_pi : quarter_pi;          // (+inf,+inf)->+pi/4 ; (-inf,+inf)->-pi/4
      else         return y.sign ? -(pi - quarter_pi) : (pi - quarter_pi); // (+inf,-inf)->+3pi/4 ; (-inf,-inf)->-3pi/4
    }
    if (y.is_inf()) {
      return y.sign ? -half_pi : half_pi;
    }
    // x is inf, y finite
    if (!x.sign) {
      // atan2(y, +inf) -> ±0 (signed like y)
      return make_zero(y.sign);
    } else {
      // atan2(y, -inf) -> ±pi (signed like y)
      return y.sign ? -pi : pi;
    }
  }

  // Both zero -> NaN (undefined)
  if (x.is_zero() && y.is_zero()) return make_qnan(1);

  // y == 0
  if (y.is_zero()) {
    const ext80 pi = ext80::from_double(3.141592653589793238462643383279502884);
    if (!x.sign) return y;                // atan2(±0, +x) -> ±0
    return y.sign ? -pi : pi;             // atan2(±0, -x) -> ±pi
  }
  ...
  // theta = atan(r) in [0, pi/2)
  ext80 theta = atan_pos(r);
  ...
  if (!x.sign) {
    // x > 0
    return y.sign ? -theta : theta;
  } else {
    // x < 0
    if (!y.sign) return pi - theta;       // y > 0
    else         return theta - pi;       // y < 0
  }
}

private:
// atan(u) for |u| <= (sqrt(2)-1) using odd Taylor series with Horner form.
// This converges quickly on that interval.
static X87_EXT80_FORCEINLINE ext80 atan_small_series(ext80 u) {
  // atan(u) = u - u^3/3 + u^5/5 - u^7/7 + ...  (odd terms)
  // We'll use terms up to u^21/21 (11 odd terms). On |u|<=0.4142, this is very accurate.
  ext80 uu = u * u;

  // Horner on uu:
  // atan(u) = u * (1 + uu*( -1/3 + uu*( 1/5 + uu*( -1/7 + ... )))
  // Coefficients as double are fine; they get converted to ext80.
  ext80 c21 = ext80::from_double( 1.0/21.0);
  ext80 c19 = ext80::from_double(-1.0/19.0);
  ext80 c17 = ext80::from_double( 1.0/17.0);
  ext80 c15 = ext80::from_double(-1.0/15.0);
  ext80 c13 = ext80::from_double( 1.0/13.0);
  ext80 c11 = ext80::from_double(-1.0/11.0);
  ext80 c9  = ext80::from_double( 1.0/9.0);
  ext80 c7  = ext80::from_double(-1.0/7.0);
  ext80 c5  = ext80::from_double( 1.0/5.0);
  ext80 c3  = ext80::from_double(-1.0/3.0);
  ...
  return u * (ext80::from_double(1.0) + uu * p);
}

// atan(z) for z >= 0 (portable reductions)
static X87_EXT80_FORCEINLINE ext80 atan_pos(ext80 z) {
  // constants
  const ext80 one = ext80::from_double(1.0);
  const ext80 pi_over_2 = ext80::from_double(1.570796326794896619231321691639751442);
  const ext80 pi_over_4 = ext80::from_double(0.785398163397448309615660845819875721);

  // t = sqrt(2)-1 ~= 0.41421356237...
  // Using a double literal here is fine; it just sets the reduction boundary.
  const ext80 t = ext80::from_double(0.4142135623730950488);

  // z > 1: atan(z) = pi/2 - atan(1/z)
  // (avoid compare helper; use your exp/sig compare via existing compare() if present)
  if (compare(z, one) == 0 /* greater */) {
    ext80 inv = one / z;
    return pi_over_2 - atan_pos(inv);
  }

  // z in (t, 1]: atan(z) = pi/4 + atan((z-1)/(z+1))
  if (compare(z, t) == 0 /* greater */) {
    ext80 u = (z - one) / (z + one);
    return pi_over_4 + atan_small_series(u);
  }

  // z in [0, t]
  return atan_small_series(z);
}
