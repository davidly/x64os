...
do
    ...
    _clangbuild="clang-18 -x c++ "$1".c -m32 -o x32clangbin"$optflag"/"$1" -O"$optflag" -static -Wno-implicit-const-int-float-conversion -fsigned-char -Wno-format -Wno-format-security -std=c++14 -lm -lstdc++"
    _gnubuild="g++ "$1".c -m32 -U_FORTIFY_SOURCE -o x32bin"$optflag"/"$1" -O"$optflag" -static -fsigned-char -Wno-format -Wno-format-security"
