...
#define array_operations_test( ftype, dim ) \
    ...
    { \
        ...
        mul_##ftype##dim(); \
        /* printf( "mul: " ); print_array_##ftype##dim(); */ \
        div_##ftype##dim(); \
        /* printf( "div: " ); print_array_##ftype##dim(); */ \
        ftype sum = sum_##ftype##dim(); \
        ftype magnitude = magnitude_##ftype##dim(); \
        printf( "type %s size %d, sum %.0Lf, magnitude %.0Lf min, %.0Lf max %.0Lf\n", #ftype, dim, (long double) sum, (long double) magnitude, \
                (long double) min_##ftype##dim(), (long double) max_##ftype##dim() ); \
        fillA_##ftype##dim( -10 ); \
        ...
        { \
            printf( "ERROR! n sum differs. type %s size %d, sum_n %.0Lf, magnitude %.0Lf, min, %.0Lf max %.0Lf\n", \
                    #ftype, dim, (long double) sum_n, (long double) magnitude, (long double) min_##ftype##dim(), (long double) max_##ftype##dim() ); \
            exit( 1 ); \
        } \
        ...
        { \
            printf( "ERROR! n magnitude differs. type %s size %d, sum %.0Lf, magnitude_n %.0Lf, min, %.0Lf max %.0Lf\n", \
                    #ftype, dim, (long double) sum, (long double) magnitude_n, (long double) min_##ftype##dim(), (long double) max_##ftype##dim() ); \
            exit( 1 ); \
        } \
        ...
        if ( aGEb != ( aGTb + aEQb ) ) \
        { \
            printf( "ERROR! aGEb %.0Lf != GT %.0Lf + EQ %.0Lf\n", (long double) aGEb, (long double) aGTb, (long double) aEQb ); \
            exit( 1 ); \
        } \
        if ( aLEb != ( aLTb + aEQb ) ) \
        { \
            printf( "ERROR! aLEb %.0Lf != LT %.0Lf + EQ %.0Lf\n", (long double) aLEb, (long double) aLTb, (long double) aEQb ); \
            exit( 1 ); \
        } \
        if ( dim != ( aLTb + aGEb ) ) \
        { \
            printf( "ERROR! aLTb %.0Lf + GE %.0Lf != dim %u\n", (long double) aLTb, (long double) aGEb, dim ); \
            exit( 1 ); \
        } \
        ...
        if ( sumC != sumD ) \
        { \
            printf( "ERROR! after mul sumC %.0Lf, sumD %.0Lf\n", (long double) sumC, (long double) sumD ); \
            exit( 1 ); \
        } \
        ...
        if ( sumC != sumD ) \
        { \
            printf( "ERROR! after mul div2 sumC %.0Lf, sumD %.0Lf\n", (long double) sumC, (long double) sumD ); \
            exit( 1 ); \
        } \
        ...
        if ( sumC != sumD ) \
        { \
            printf( "ERROR! after mul div2b sumC %.0Lf, sumD %.0Lf\n", (long double) sumC, (long double) sumD ); \
            exit( 1 ); \
        } \
        ...
        if ( sumC != sumD ) \
        { \
            printf( "ERROR! after div sumC %.0Lf, sumD %.0Lf, type %s, dim %u\n", (long double) sumC, (long double) sumD, #ftype, dim ); \
            exit( 1 ); \
        } \
        inc_C_##ftype##dim(); \
        dec_C_##ftype##dim(); \
        ...
        if ( sumC != sumD ) \
        { \
            printf( "ERROR! after sqrt/sqr sumC %.0Lf, sumD %.0Lf, type %s, dim %u\n", (long double) sumC, (long double) sumD, #ftype, dim ); \
            exit( 1 ); \
        } \
        return sum; \
    }
